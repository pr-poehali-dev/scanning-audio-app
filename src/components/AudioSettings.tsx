...
export const AudioSettings = ({
  ...
  const loadFilesForVariant = async (variant: 'v1' | 'v2') => {
    ...
    try {
      ...
      // НЕ ОЧИЩАЕМ всё хранилище! Сохраняем файлы других вариантов
      console.log('💾 Обновление файлов в локальном хранилище...');
      
      // Сохраняем только отфильтрованные файлы для текущего варианта
      // Файлы других вариантов остаются нетронутыми
              ...
              <Button
                ...
                onClick={async () => {
                  if (audioSettings.variant !== 'v1') {
                    ...
                    // Автосохранение в localStorage происходит через useEffect
                    // Загружаем файлы для варианта 1
                    await loadFilesForVariant('v1');
                    console.log('✅ Вариант 1 активирован');
                  }
                }}
              ...
              <Button
                ...
                onClick={async () => {
                  if (audioSettings.variant !== 'v2') {
                    ...
                    setAudioSettings(newSettings);
                    // Автосохранение в localStorage происходит через useEffect
                    // Загружаем файлы для варианта 2
                    await loadFilesForVariant('v2');
                    console.log('✅ Вариант 2 активирован');
                  }
                }}
                disabled={isLoadingVariant}
                className={`w-full h-14 text-base font-semibold ${
